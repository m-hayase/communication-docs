...
const problems = [
  { icon: '⚡', num: '290兆円', label: '日本の雇用者報酬', desc: 'この巨大市場は未だ人間労働に依存し、構造的な非効率を抱えている' },
  { icon: '📉', num: '69%', label: 'DX未着手企業', desc: '中小企業の約7割がDXに未着手。業務のデジタル化すら進んでいない現実', note: '※中小企業庁調べ' },
  { icon: '🔄', num: '47%', label: '定型業務の割合', desc: 'ホワイトカラーの業務時間の約半分が、AIで代替可能な定型作業に費やされている' },
]

const solutions = [
  { num: '01', title: '仮想業務空間', desc: 'AIが業務を自律的に遂行する新しいワークスペース。人間は意思決定と創造に集中する。' },
  { num: '02', title: '業務全領域カバー', desc: '営業・会計・人事・購買——断片化したSaaSを統合し、すべての業務をひとつの空間で完結させる。' },
  { num: '03', title: 'AI-Native設計', desc: '既存SaaSへのAI後付けではない。AIが前提のアーキテクチャで、業務プロセスそのものを再定義する。' },
]

const advantages = [
  { value: '2週間', label: 'プロトタイプ開発', desc: 'アイデアから動くプロダクトまで。AI駆動の開発速度。' },
  { value: 'Day 1', label: 'AI-Native', desc: '後付けAIではない。設計思想からAIファースト。' },
  { value: '全業務', label: 'ワンプラットフォーム', desc: '断片化した業務SaaSを統合する唯一の選択肢。' },
]
...
<div class="page" :class="{ loaded }">
  ...
  <section class="slide problem">
    <div class="problem-inner">
      ...
      <div class="problem-grid">
        <div v-for="(p, i) in problems" :key="i" class="problem-card glass-panel obs" :style="{ transitionDelay: i*0.15+'s' }">
          <div class="problem-top">
            <span class="problem-num">{{ p.num }}</span>
            <span class="problem-label">{{ p.label }}</span>
          </div>
          <p class="problem-desc">{{ p.desc }}</p>
          <span v-if="p.note" class="problem-note">{{ p.note }}</span>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="slide solution">
    <div class="solution-inner">
      <div class="sec-head obs">
        ...
        <h2>What we build</h2>
        <p class="sec-sub">AIが業務を遂行する、まったく新しいワークスペース</p>
      </div>
      <div class="solution-grid">
        <div v-for="(s, i) in solutions" :key="i" class="solution-card glass-panel obs" :style="{ transitionDelay: i*0.15+'s' }">
      ...
      <div class="solution-screenshot obs">
        <div class="screenshot-placeholder glass-panel">
          ...
          <span class="screenshot-label">Product Preview — Confidential</span>
        </div>
      </div>
    </div>
  </section>
  ...
  <section class="slide why-us">
    <div class="why-inner">
      ...
      <div class="why-grid">
        <div v-for="(a, i) in advantages" :key="i" class="why-card obs" :style="{ transitionDelay: i*0.12+'s' }">
  ...
  <section class="slide vision">
    <div class="vision-card glass-panel obs">
      ...
      <p class="vision-stage">Pre-Seed — Building the future of work</p>
    </div>
  </section>
  ...
  <section id="contact" class="slide cta">
    <div class="cta-card glass-panel obs">
      ...
      <div class="cta-buttons">
        <a href="mailto:hello@example.com" class="btn-primary btn-lg">
...
.soul {
  ...
  filter: blur(60px); transition: transform 0.5s ease-out;
}
.soul.sa {
  width: 500px; height: 500px; top: -8%; right: 5%;
  background: radial-gradient(circle, rgba(147,51,234,0.18) 0%, rgba(79,70,229,0.08) 40%, transparent 70%);
  animation: soulA 16s ease-in-out infinite;
}
.soul.sb {
  width: 400px; height: 400px; bottom: 5%; left: -3%;
  background: radial-gradient(circle, rgba(59,130,246,0.16) 0%, rgba(6,182,212,0.08) 40%, transparent 70%);
  animation: soulB 20s ease-in-out infinite;
}
.soul.sc {
  width: 350px; height: 350px; top: 35%; left: 25%;
  background: radial-gradient(circle, rgba(236,72,153,0.15) 0%, rgba(244,114,182,0.07) 40%, transparent 70%);
  animation: soulC 14s ease-in-out infinite;
}
.soul.sd {
  width: 250px; height: 250px; top: 60%; right: 15%;
  background: radial-gradient(circle, rgba(245,158,11,0.14) 0%, rgba(251,191,36,0.06) 40%, transparent 70%);
  animation: soulD 18s ease-in-out infinite;
}
.soul.se {
  width: 300px; height: 300px; top: 10%; left: 45%;
  background: radial-gradient(circle, rgba(16,185,129,0.13) 0%, rgba(52,211,153,0.06) 40%, transparent 70%);
...
.wisp.wA {
  width: 700px; height: 100px; top: 28%; left: -5%;
  background: linear-gradient(90deg, transparent 0%, rgba(147,51,234,0.1) 30%, rgba(79,70,229,0.12) 50%, rgba(147,51,234,0.1) 70%, transparent 100%);
  border-radius: 50%; animation: wispA 14s ease-in-out infinite;
}
.wisp.wB {
  width: 600px; height: 80px; top: 58%; right: -8%;
  background: linear-gradient(90deg, transparent 0%, rgba(59,130,246,0.08) 30%, rgba(236,72,153,0.1) 50%, rgba(59,130,246,0.08) 70%, transparent 100%);
  border-radius: 50%; animation: wispB 18s ease-in-out infinite;
}
.wisp.wC {
  width: 500px; height: 70px; top: 78%; left: 15%;
  background: linear-gradient(90deg, transparent 0%, rgba(16,185,129,0.07) 30%, rgba(245,158,11,0.09) 50%, rgba(16,185,129,0.07) 70%, transparent 100%);
  border-radius: 50%; animation: wispC 20s ease-in-out infinite;
}
...
.sec-head h2 {
  ...
  font-weight: 600; letter-spacing: -0.02em; line-height: 1.5;
}
.sec-sub {
  font-size: 0.95rem; color: #888; margin-top: 0.8rem; letter-spacing: 0.02em;
}
...
.btn-primary {
  ...
  border: 1px solid #111; border-radius: 100px;
...
.hero-label {
  ...
  margin-bottom: 2rem; font-size: 0.82rem; letter-spacing: 0.2em; color: #999;
}
.label-line {
...
.hero-sub {
  ...
  margin-bottom: 0.8rem; opacity: 0; transform: translateY(25px);
...
/* ===== 2. PROBLEM ===== */
.problem { flex-direction: column; padding: 4rem 2rem; }
.problem-inner { max-width: 1000px; width: 100%; }
.problem-grid {
...
.problem-top { margin-bottom: 1.2rem; }
.problem-num {
  display: block; font-size: clamp(2rem, 3.5vw, 2.8rem);
  font-weight: 700; letter-spacing: -0.02em; color: #111;
}
.problem-label {
  display: block; font-size: 0.75rem; letter-spacing: 0.12em;
  color: #999; margin-top: 0.3rem; text-transform: uppercase;
}
.problem-desc { font-size: 0.88rem; line-height: 1.8; color: #666; }
.problem-note {
  display: block; font-size: 0.65rem; color: #bbb; margin-top: 0.6rem;
}

/* ===== 3. SOLUTION ===== */
.solution { flex-direction: column; padding: 4rem 2rem; }
.solution-inner { max-width: 1100px; width: 100%; }
.solution-grid {
  display: grid; grid-template-columns: repeat(3, 1fr); gap: 1.5rem;
  margin-bottom: 3rem;
}
...
.solution-card h3 { font-size: 1.25rem; font-weight: 600; margin-bottom: 0.8rem; }
.solution-card p { font-size: 0.9rem; line-height: 1.85; color: #666; }
.feat-accent {
  position: absolute; bottom: 0; left: 0;
  width: 0; height: 2px;
  background: linear-gradient(90deg, rgba(147,51,234,0.4), rgba(59,130,246,0.3), transparent);
  transition: width 0.6s cubic-bezier(0.16,1,0.3,1);
}
.solution-card:hover .feat-accent { width: 100%; }

/* Screenshot preview (blurred mock) */
.solution-screenshot { text-align: center; }
.screenshot-placeholder {
  display: inline-block; padding: 1.5rem; max-width: 700px; width: 100%;
}
.screenshot-blur {
...
.screenshot-mock {
  display: flex; height: 200px; background: #f0f0f0; border-radius: 8px; overflow: hidden;
}
.mock-sidebar {
  width: 60px; background: linear-gradient(180deg, #e8e8e8, #ddd);
}
.mock-main { flex: 1; padding: 12px; display: flex; flex-direction: column; gap: 8px; }
.mock-bar {
  height: 24px; background: #e0e0e0; border-radius: 6px;
}
.mock-content {
...
.screenshot-label {
  display: block; font-size: 0.7rem; color: #bbb;
  letter-spacing: 0.1em; margin-top: 1rem;
}

/* ===== 4. WHY US ===== */
.why-us { flex-direction: column; padding: 4rem 2rem; }
.why-inner { max-width: 1000px; width: 100%; }
.why-grid {
  display: grid; grid-template-columns: repeat(3, 1fr); gap: 2rem;
}
.why-card { text-align: center; }
.why-value {
  display: block; font-size: clamp(2.5rem, 5vw, 3.5rem);
  font-weight: 700; letter-spacing: -0.03em; color: #111;
}
.why-label {
  display: block; font-size: 0.8rem; letter-spacing: 0.15em;
  color: #999; text-transform: uppercase; margin: 0.3rem 0 1rem;
}
.why-desc { font-size: 0.9rem; line-height: 1.7; color: #666; }

/* ===== 5. VISION / TEAM ===== */
.vision { padding: 4rem 2rem; }
.vision-card {
  max-width: 650px; width: 100%; padding: 4rem 3.5rem; text-align: center;
}
.vision-quote {
  font-size: clamp(1.2rem, 2.5vw, 1.7rem);
  font-weight: 500; line-height: 1.7; color: #222;
  margin: 1.5rem 0 2.5rem; font-style: normal;
...
.founder-name {
  display: block; font-size: 0.95rem; font-weight: 600; color: #111;
}
.founder-role {
  display: block; font-size: 0.75rem; color: #999; letter-spacing: 0.06em;
}
.vision-stage {
  font-size: 0.78rem; color: #bbb; letter-spacing: 0.1em;
...
.cta-card {
  max-width: 600px; width: 100%; padding: 4rem 3rem; text-align: center;
}
.cta-card h2 {
  font-size: clamp(2rem, 4vw, 3rem);
  font-weight: 700; letter-spacing: -0.03em; margin-bottom: 1.2rem;
}
.cta-desc {
  font-size: 0.95rem; line-height: 1.8; color: #666; margin-bottom: 2.5rem;
}
.cta-buttons { margin-bottom: 1.5rem; }
.cta-note { font-size: 0.78rem; color: #bbb; letter-spacing: 0.04em; }
...
.foot-wrap { width: 100%; padding: 3rem 2rem; }
.marquee-wrap {
  width: 100%; overflow: hidden; padding: 1.8rem 0;
